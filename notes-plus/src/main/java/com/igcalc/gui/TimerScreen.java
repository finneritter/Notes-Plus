...
public class TimerScreen extends Screen {
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        if (keybindCardVisible) {
            ...
            String[][] rows = (currentMode == Mode.STOPWATCH)
                ? new String[][] {
                    {"Start/Stop",  "Space"},
                    {"Lap",         "L"},
                    {"Reset",       "R"},
    ...
    public boolean keyPressed(KeyInput input) {
        ...
        if (keyCode == GLFW.GLFW_KEY_TAB) {
            currentMode = (currentMode == Mode.TIMER) ? Mode.STOPWATCH : Mode.TIMER;
            syncToHud();
            return true;
        }
        ...
        if (currentMode == Mode.STOPWATCH) {
            ...
            // L: record lap
            if (keyCode == GLFW.GLFW_KEY_L && swState == SwState.RUNNING) {
                swLaps.add(getSwElapsedMs());
                syncToHud();
                return true;
            }
            // R or Delete: reset
            if (keyCode == GLFW.GLFW_KEY_R || keyCode == GLFW.GLFW_KEY_DELETE) {
        ...
        if (keyCode == GLFW.GLFW_KEY_SPACE) {
            if (state == State.IDLE && getTotalSeconds() > 0) {
                startTimer(); syncToHud(); return true;
            } else if (state == State.ACTIVE) {
                pauseTimer(); syncToHud(); return true;
            } else if (state == State.PAUSED) {
                resumeTimer(); syncToHud(); return true;
            }
        }
        // R: cancel/reset active or paused timer
        if (keyCode == GLFW.GLFW_KEY_R && (state == State.ACTIVE || state == State.PAUSED || state == State.COMPLETE)) {
            state = State.IDLE;
            syncToHud();
            return true;
        }
