...
public class IgCalcSettingsScreen extends Screen {
    ...
    private static final int TOTAL_H         = 840;  // total content height

    // Offset from winY where the color TextFieldWidgets start
    // = TITLE_BAR_HEIGHT(18) + 10 + 4*(ROW_H+ROW_PAD)(96) + divider(10) + 5*(ROW_H+ROW_PAD)(120) + divider(10) + label(12) + hudBtn(20)
    private static final int COLOR_SECTION_OFFSET = 320;
    ...
    private int getMaxScroll() {
        int visH = getVisH();
        // Content below title bar: TOTAL_H - TITLE_BAR_HEIGHT - 36 (bottom buttons)
        // Visible: visH - TITLE_BAR_HEIGHT - 36
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        int clipBottom = winY + visH - 36;  // leave room for bottom buttons
        ...
        renderReadOnlyRow(ctx, contentY, labelX, btnX, btnW,
                Text.translatable("igcalc.config.key.openCalc").getString(), "F6 \u2014 Controls");
        contentY += ROW_H + ROW_PAD;

        renderReadOnlyRow(ctx, contentY, labelX, btnX, btnW,
                Text.translatable("igcalc.config.key.openNotes").getString(), "F7 \u2014 Controls");
        contentY += ROW_H + ROW_PAD;

        renderReadOnlyRow(ctx, contentY, labelX, btnX, btnW,
                Text.translatable("igcalc.config.key.openTimer").getString(), "F8 \u2014 Controls");
        contentY += ROW_H + ROW_PAD;

        renderReadOnlyRow(ctx, contentY, labelX, btnX, btnW,
                Text.translatable("igcalc.config.key.hudToggle").getString(), "F9 \u2014 Controls");
        contentY += ROW_H + ROW_PAD;

        renderReadOnlyRow(ctx, contentY, labelX, btnX, btnW,
                Text.translatable("igcalc.config.key.hudInteract").getString(), "Alt \u2014 Controls");
        contentY += ROW_H + ROW_PAD;
    ...
    private void renderRow(DrawContext ctx, int mouseX, int mouseY,
        ...
        boolean hovered   = !capturing && mouseX >= btnX && mouseX < btnX + btnW
                         && mouseY >= y && mouseY < y + ROW_H;
        int bgCol = capturing ? COLOR_BTN_CAP : (hovered ? COLOR_BTN_HOVER : COLOR_BTN_NORM);
        fillRoundedRect(ctx, btnX, y, btnW, ROW_H - 2, 3, bgCol);

        String btnLabel = capturing
                ? Text.translatable("igcalc.config.capturing").getString()
                : binding.display();
        int textX = btnX + (btnW - textRenderer.getWidth(btnLabel)) / 2;
        ctx.drawText(textRenderer, btnLabel, textX,
                y + (ROW_H - textRenderer.fontHeight) / 2, COLOR_BTN_TEXT, false);
    }
    ...
    public boolean mouseClicked(Click click, boolean propagated) {
        ...
        for (int k = 0; k < 6; k++) {
            ...
            if (mx >= previewX2 && mx < previewX2 + 12 && my >= sqY && my < sqY + 12) {
        ...
        String[] keys = { "keyNewNote", "keyFindSidebar", "keyQuickNote", "keyInsertCoords", "keyCopyClipboard" };
        for (String key : keys) {
            if (mx >= btnX && mx < btnX + btnW && my >= contentY && my < contentY + ROW_H) {
                capturingFor = key;
                return true;
            }
            contentY += ROW_H + ROW_PAD;
    ...
    public boolean keyPressed(KeyInput input) {
        ...
        if (capturingFor != null) {
            ...
            switch (capturingFor) {
                case "keyNewNote"       -> bndNewNote       = nb;
                case "keyFindSidebar"   -> bndFindSidebar   = nb;
                case "keyQuickNote"     -> bndQuickNote     = nb;
                case "keyInsertCoords"  -> bndInsertCoords  = nb;
                case "keyCopyClipboard" -> bndCopyClipboard = nb;
            }
            capturingFor = null;
            return true;
        }
