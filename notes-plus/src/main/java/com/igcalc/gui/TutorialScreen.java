...
public class TutorialScreen {
    ...
    /** Returns dynamic step title, reading current keybinds. */
    private static String getStepTitle(int step) {
        return switch (step) {
            case 1  -> "Notes (" + IgCalcClient.notesKey.getBoundKeyLocalizedText().getString() + ")";
            case 6  -> "Calculator (" + IgCalcClient.calculatorKey.getBoundKeyLocalizedText().getString() + ")";
            case 8  -> "Timer (" + IgCalcClient.timerKey.getBoundKeyLocalizedText().getString() + ")";
            default -> STEPS[step].title;
        };
    }

    /** Returns dynamic step text, reading current keybinds. */
    private static String getStepText(int step) {
        IgCalcConfig cfg = IgCalcConfig.getInstance();
        return switch (step) {
            case 1  -> "Press **" + IgCalcClient.notesKey.getBoundKeyLocalizedText().getString()
                     + "** to open Notes. Write and organize notes that save automatically per-world.";
            case 6  -> "Press **" + IgCalcClient.calculatorKey.getBoundKeyLocalizedText().getString()
                     + "** for an in-game calculator. Supports +, \u2212, \u00d7, \u00f7, parentheses, and keyboard input.";
            case 8  -> "Press **" + IgCalcClient.timerKey.getBoundKeyLocalizedText().getString()
                     + "** for a timer and stopwatch. Switch modes with the tabs at the top.";
            case 10 -> "Click the **pin \u2691** button to keep notes or timer on your HUD. Toggle visibility with **"
                     + IgCalcClient.hudToggleKey.getBoundKeyLocalizedText().getString()
                     + "**. Hold **" + IgCalcClient.hudInteractKey.getBoundKeyLocalizedText().getString()
                     + "** to get a cursor and interact with pinned notes \u2014 scroll, switch files with **Ctrl+Left/Right**, and search with **Ctrl+Enter**.";
            case 11 -> "**" + cfg.keyQuickNote.display() + "** (quick note), **"
                     + cfg.keyInsertCoords.display() + "** (insert coords), **"
                     + cfg.keyCopyClipboard.display() + "** (copy to clipboard), **"
                     + cfg.keyStopwatchToggle.display()
                     + "** (stopwatch toggle). Customize all keybinds in Mod Menu \u203a Notes+ \u203a Settings.";
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        String stepText  = getStepText(currentStep);
        ...
        List<String> wrappedTitle = wrapText(stepTitle, innerW);
        List<String> wrappedText  = wrapText(stepText, innerW);
